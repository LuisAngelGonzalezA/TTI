...
def insert_panel(self):
		...
		img = Tk.PhotoImage(file="/home/luis/Documentos/pruebas_python/sistema/panel2.gif")
...
def nombre_tener(self):
	...
	if len(var) == 0:
		...
		if resultado == 0:
			...
			consulta="insert into panel_solar values(null,1,'{nombre}',{voltaje},{corriente})".format(nombre=var,voltaje=var1,corriente=var2)
			print(consulta)
			mysql=mysql_conection.mysql_conexion_tornasol()
			cursor = mysql.cursor()
			resultado=cursor.execute(consulta)
...
def insert_bateria(self):
		...
		tem=Tk.Label(self.ventana_insert_panel, text="Temperatura Máxima:",font="Arial 14",justify=Tk.CENTER) 
